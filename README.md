...
Turn a **JSON object** or a **JSON5 object literal string** into **dotenv-style** text: one `KEY=value` per line, ending with newlines, ready for `.env` files or tools that read the same format.

```ts
import { jsonToEnv } from "@sourcepride/json-to-env";

const dotenv = jsonToEnv({ port: 3000, debug: true });
// PORT=3000\nDEBUG=true\n
```
...
|----------|------|--------|
| `input` | `Record<string, unknown>` **or** `string` | Either a plain JavaScript object, or a **trimmed** JSON5 string that parses to an object (e.g. `{ port: 3000 }`). |
