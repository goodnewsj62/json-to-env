 ...
 * ignored).
 */
export function buildCleanKeys(key, seperator, keyCase = "upper_snake") {
 ...
 */
export function buildEnvKeyFromSeperator(parts, seperator, keyCase = "upper_snake") {
    if (seperator) {
    ...
    switch (keyCase) {
        case "camel_case": {
            ...
            const others = parts
                .slice(1)
                .reduce((cumm, value) => cumm + value.substring(0, 1).toUpperCase() + value.substring(1), "");
            return parts[0] + others;
        }
        case "pascal_case": {
            return parts.reduce((cumm, value) => cumm + value.substring(0, 1).toUpperCase() + value.substring(1), "");
        }
        case "upper_snake": {
...
export function keyToEnvKeyPrefix(key) {
    const cleaned = replaceIllegalChar(key).trim();
    if (cleaned.length === 0)
        return "";
